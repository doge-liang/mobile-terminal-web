...
> 状态(2026-07-16):设计与实现均已完成(Tasks 1-10,代码 + 单测)。控制面凭证配置(Task 4,建 R2 桶 + token)与跨节点端到端验收(Task 9)、`box-snapshot.timer`/`box-prune.timer` 实际启用(Task 11)尚未跑完——本文档描述的是**已实现的行为**,不代表已在两节点真实环境跑通全部验收标准。凭证就位、验收通过后本节会更新。
...
**`RESTIC_PASSWORD` 丢失 = 全部快照永久不可解密**(restic 仓库 AES-256 加密,没有找回机制)。生成后务必单独离线备份一份(密码管理器/纸面),不要只留在这台机器的 env 文件里。
...
- `box status` —— **尚未实现**(`box/bin/box` 里当前是占位,调用会报"尚未实现";留待控制面凭证就位、Task 4/9 收尾时补上节点身份/R2 连通/restic 统计/timer 状态的汇总)。目前判断状态靠 `box ls` + 下面几条手动核对。
